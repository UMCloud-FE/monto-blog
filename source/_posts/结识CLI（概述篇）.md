...
您可能也知道，仅仅这样是不太行了，终端是不会解析全局指令 `yarn` 的，因为他不知道这个目录。系统是通过全局变量来找路径的，我们看一下：
...
那么我们自己的包如果是本地开发要怎么做呢？如果有多个依赖包，他们会分别被软连接吗？发布的时候是否需要配置软链接？学完本系列课程，相信您会有更多的思考。
